/*
	feWorksheet(Javascript)
	Include: jQuery v2.1.0 | (c) 2005, 2014 jQuery Foundation, Inc. | jquery.org/license
	Version: 1.2
*/
!(function (a, b) {
            ...
            if (r && (i = r.events)) {
                ...
                while (j--)
                    if (((h = Y.exec(b[j]) || []), (n = q = h[1]), (p = (h[2] || "").split(".").sort()), n)) {
                        ...
                        g &&
                            !m.length &&
                            ((l.teardown && l.teardown.call(a, p, r.handle) !== !1) || o.removeEvent(a, n, r.handle),
                            delete i[n]);
            ...
            remove: function (a, b) {
                for (var c, d = a ? o.filter(a, this) : this, e = 0; null != (c = d[e]); e++)
                    b || 1 !== c.nodeType || o.cleanData(ob(c)),
                        c.parentNode &&
                            (b && o.contains(c.ownerDocument, c) && mb(ob(c, "script")), c.parentNode.removeChild(c));
                return this;
            },
            empty: function () {
                ...
                return (
                    ...
                    this.each(function () {
                        var b,
                            ...
                            g = d ? d.length : 0;
                        for (
                            c.finish = !0, o.queue(this, a, []), e && e.stop && e.stop.call(this, !0), b = f.length;
                            b--;
    ...
    o.fn.extend({
        ...
        o.each(o.expr.match.bool.source.match(/\w+/g), function (a, b) {
            ...
            $b[b] = function (a, b, d) {
                var e, f;
                return (
                    d || ((f = $b[b]), ($b[b] = e), (e = null != c(a, b, d) ? b.toLowerCase() : null), ($b[b] = f)), e
                );
            };
        });
...
(function ($, window, document, undefined) {
    var feWorksheet = {
        init: function (options) {
            ...
            return this.each(function () {
                $("title").text($("h1").text());
                $("." + opt.filterSectionClass).feWorksheet("_taskSetPage", opt);
                $(opt.workseetEl)
                    .feWorksheet("setNavi", opt)
                    .feWorksheet("setNumber", opt)
                    .feWorksheet("reflectDepth", opt)
                    .find("td." + opt.nameCellClass)
                    .feWorksheet("_linkPath", opt);
                if (opt.delayInfo != false) $("td." + opt.dueCellClass).feWorksheet("delayInfo", opt);
            });
        },
        setNavi: function (opt) {
            var _naviCode = "<nav>" + "<h2>" + opt.naviTitle + "</h2>" + "<ul></ul>" + "</nav>";
            $("h1").after(_naviCode);
            return this.each(function () {
                var $this = $(this),
                    $navEl = $("nav"),
                    $menuTitEl = $this.prev("h3"),
                    thisIdx = $(opt.workseetEl).index($this) + 1;
                $menuTitEl.attr("id", opt.naviPrefix + thisIdx);
                $navEl
                    .find("ul")
                    .append('<li><a href="#' + opt.naviPrefix + thisIdx + '">' + $menuTitEl.text() + "</a></li>");
            });
        },
        setNumber: function (opt) {
            return this.each(function () {
                var $this = $(this),
                    _theadCellCode = '<th scope="col" class="' + opt.numCellClass + '">' + opt.numCellText + "</th>";
                $this
                    .find("thead tr")
                    .prepend(_theadCellCode)
                    .end()
                    .find("tbody tr")
                    .each(function () {
                        var idxNum = $(this).index() + 1,
                            _tbodyCellCode = '<td class="' + opt.numCellClass + '">' + idxNum + "</td>";
                        $(this).prepend(_tbodyCellCode);
                    });
            });
        },
        reflectDepth: function (opt) {
            return this.each(function () {
                var $this = $(this),
                    $tbodyTrEl = $this.find("tbody tr"),
                    $depthCell = $tbodyTrEl.find("." + opt.depthCellClass);
                $tbodyTrEl.each(function () {
                    $(this)
                        .find($depthCell)
                        .each(function () {
                            var thisIdx = $(this).index() - 1,
                                reflectText = $(this).closest($tbodyTrEl).prev().find($depthCell).eq(thisIdx).text();
        ...
        delayInfo: function (opt) {
            return this.each(function () {
                var $this = $(this),
                    dDate = $this.text(),
                    endDate = $this.next().text(),
                    mdDate = $this.next().next().text(),
                    tDate = new Date(),
                    dFormat = /(\d{2})-(\d{2})-(\d{2})/,
                    tDate =
                        String(tDate.getFullYear()).slice(2) +
                        "-" +
                        (tDate.getMonth() + 1 < 10 ? "0" + (tDate.getMonth() + 1) : "" + (tDate.getMonth() + 1)) +
                        "-" +
                        (tDate.getDate() < 10 ? "0" + tDate.getDate() : "" + tDate.getDate());
                    console.log(mdDate);
                    if (mdDate ) $this.addClass(opt.delayModify);
                    else if (endDate ) $this.addClass(opt.delayBefore);
                    //else if (dDate == "" || !dFormat.test(dDate)) $this.addClass(opt.delayError);
                    //else if (dDate > tDate ) $this.addClass(opt.delayBefore);
                    else if (dDate < tDate) $this.addClass(opt.delayAfter);
                    else if (dDate == tDate) $this.addClass(opt.delayToday);
                    //else $this.addClass(opt.delayError);
            });
        },
        _taskSetPage: function (opt) {
            return this.each(function () {
                var $this = $(this),
                    $taskEl = $this.find("label:has(b):has(i)"),
                    $_taskCell = $("td." + opt.taskCellClass),
                    $_checkBoxEl = $this.find(":checkbox"),
                    taskPrefix = "taskVal",
                    defaultTask = $this.find(".default").text().trim(),
                    taskKeyArray = $taskEl
                        .map(function () {
                            return $(this).find("b").text().trim();
                        })
                        .get(),
                    taskNameArray = $taskEl
                        .map(function () {
                ...
                $taskEl.each(function () {
                    ...
                    $(this)
                        .find(":checkbox")
                        .attr("value", taskPrefix + taskKey);
                });
                $_checkBoxEl
                    .on("change", function () {
                        var $_tgTableRow = $(opt.workseetEl).find("tbody > tr");
                        if ($(this).val() == "all") {
                            if (!$(this).is(":checked")) {
                                $_checkBoxEl.removeAttr("disabled").not($(this)).trigger("click");
                            } else {
                                $_checkBoxEl.not($(this)).attr("disabled", "disabled").removeAttr("checked");
                                $_tgTableRow.show();
                            }
                        } else {
                            var tgDataVal = $(this).val().split(taskPrefix)[1],
                                $tgTableRow = $_tgTableRow.filter(':has([data-task="' + tgDataVal + '"])');
                            !$(this).is(":checked") ? $tgTableRow.hide() : $tgTableRow.show();
                        }
                    })
                    .filter("[value=all]")
                    .trigger("click");
                $_taskCell.each(function () {
                    $(this).text().trim() == "" || $.inArray($(this).text(), taskKeyArray) == -1
                        ? $(this).attr("data-task", defaultTask).text(defaultTask)
                        : $(this).attr("data-task", $(this).text());
                ...
                $.each(taskKeyArray, function (idx, val) {
                    var $footerEl = $("footer"),
                        taskSum = $_taskCell.filter("[data-task=" + val + "]").size(),
                        _footerThead =
                            '<th scope="col">' + "<i>" + taskNameArray[idx] + "</i>" + "<b>" + val + "</b>" + "</th>";
                    $footerEl
                        .find("thead th:last")
                        .before(_footerThead)
                        .end()
                ...
                $("footer")
                    .find("tfoot tr")
                    .append("<td>" + $("td." + opt.taskCellClass).size() + "</td>");
                $(opt.workseetEl).feWorksheet("_taskSetTable", opt, taskKeyArray, taskNameArray);
            });
        },
        _taskSetTable: function (opt, taskKeyArray, taskNameArray) {
            return this.each(function () {
                var $this = $(this),
                    $footerEl = $("footer"),
                    $taskCell = $this.find("td." + opt.taskCellClass),
                    colSpanSize = $this.find("thead th").size(),
                    totalSize = $this.find("tbody > tr").size(),
                    menuTitle = $this.prev("h3").text(),
                    tableIdx = $(opt.workseetEl).index($this),
                    _totalCode =
                        "<tr>" +
                        ...
                        "</td>" +
                        "</tr>",
                    _footerTbodyCode = "<tr>" + '<th scope="row">' + menuTitle + "</th>" + "</tr>";
                $footerEl.find("tbody").append(_footerTbodyCode);
                $this.append("<tfoot />");
                $.each(taskKeyArray, function (idx, val) {
                    var tgTaskSize = $taskCell.filter("[data-task=" + val + "]").size(),
                        _footerCode = "<td>" + tgTaskSize + "</td>",
                        _tfootCode =
                            "<tr>" +
                            '<th scope="row" colspan="' +
                            colSpanSize +
                            ...
                            "</td>" +
                            "</tr>";
                    $footerEl.find("tbody tr:eq(" + tableIdx + ")").append(_footerCode);
                    $this.find("tfoot").append(_tfootCode);
                });
                $footerEl.find("tbody tr:eq(" + tableIdx + ")").append("<td>" + totalSize + "</td>");
                $this.find("tfoot").append(_totalCode);
            });
        },
        _linkPath: function (opt) {
            return this.each(function () {
                var $this = $(this),
                    thisTxt = $this.text(),
                    defaultPath = $this.closest(opt.workseetEl).find("caption").text().trim(),
                    folderPath = $this
                        .siblings("." + opt.pathCellClass)
                        .text()
                        .trim(),
                    newPath;
                if (!thisTxt || $this.has("a").size() != 0) return;
                !folderPath
                    ? (newPath = defaultPath + "/" + thisTxt)
                    : (newPath = defaultPath + "/" + folderPath + "/" + thisTxt);
    ...
    $.fn.feWorksheet = function (method) {
        var arguments;
        feWorksheet[method]
            ? (arguments = feWorksheet[method].apply(this, Array.prototype.slice.call(arguments, 1)))
            : (arguments = feWorksheet.init.apply(this, arguments));
        return arguments;
    };
    $.fn.feWorksheet.defaultOptions = {
        workseetEl: "table:not(footer table)",
        filterSectionClass: "filterWrap",
        numCellClass: "pageNumber",
        depthCellClass: "menuDepth",
        pathCellClass: "filePath",
        nameCellClass: "fileName",
        dueCellClass: "dueDate",
        delayBefore: "delayBefore",
        delayAfter: "delayAfter",
        delayToday: "delayToday",
        delayError: "delayError",
        delayModify: "delayModify",
        taskCellClass: "checkTask",
        naviTitle: "Navigation",
        naviPrefix: "menuIdx",
        totalCountTitle: "Total Count",
        totalCountCaption: "Task total count",
        numCellText: "Number",
        delayInfo: true,
    };
